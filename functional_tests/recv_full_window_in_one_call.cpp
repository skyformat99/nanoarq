...
TEST(functional, recv_full_window_in_one_call)
{
    arq_cfg_t cfg;
    ...
    unsigned recvd;
    arq_err_t const e = arq_recv(ctx.arq, test_output.data(), test_output.size(), &recvd);
    CHECK(ARQ_SUCCEEDED(e));
    CHECK_EQUAL(test_output.size(), recvd);
