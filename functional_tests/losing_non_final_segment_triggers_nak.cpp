...
TEST(functional, losing_non_final_segment_triggers_nak)
{
    arq_cfg_t cfg;
    ...
    // lose the first segment from the sender
    {
        void const *p;
    ...
    // drain the second segment from the sender
    {
        void const *p;
        arq_err_t e = arq_backend_send_ptr_get(sender.arq, &p, &frame_len);
        CHECK(ARQ_SUCCEEDED(e));
    ...
    // receive the second segment, drain the NAK frame
    {
