...
TEST(send_wnd, send_to_full_window_returns_zero)
{
    ...
    std::for_each(std::begin(f.msg), std::end(f.msg), [&](arq__msg_t& m) { m.len = f.wnd.msg_len; });
    f.snd.resize(16);
    int const written = arq__send_wnd_send(&f.wnd, f.snd.data(), f.snd.size());
...
TEST(send_wnd, send_more_data_than_window_space_with_partial_msg_returns_rest_of_window)
{
    ...
    f.wnd.size = 3;
    f.wnd.msg[0].len = f.wnd.msg_len;
    f.wnd.msg[1].len = f.wnd.msg_len;
    f.wnd.msg[2].len = 27;
    int const cap_in_bytes = f.wnd.cap * f.wnd.msg_len;
    int const size_in_bytes = f.wnd.msg[0].len + f.wnd.msg[1].len + f.wnd.msg[2].len;
...
{
    Fixture f;
    arq__send_wnd_ack(&f.wnd, f.wnd.seq, f.wnd.full_ack_vec - 1);
    CHECK_EQUAL(0, f.wnd.seq);
}
...
TEST(send_wnd, ack_first_seq_slides_and_increments_base_seq)
{
    Fixture f;
    f.wnd.size = 1;
    arq__send_wnd_ack(&f.wnd, f.wnd.seq, f.wnd.full_ack_vec);
    CHECK_EQUAL(1, f.wnd.seq);
}
...
TEST(send_wnd, ack_second_seq_doesnt_slide)
{
    Fixture f;
    f.wnd.size = 2;
    arq__send_wnd_ack(&f.wnd, f.wnd.seq + 1, f.wnd.full_ack_vec);
    CHECK_EQUAL(0, f.wnd.seq);
}
...
TEST(send_wnd, ack_max_seq_num_wraps_to_zero)
{
    Fixture f;
    f.wnd.size = 1;
    f.wnd.seq = ARQ__FRAME_MAX_SEQ_NUM;
    arq__send_wnd_ack(&f.wnd, f.wnd.seq, f.wnd.full_ack_vec);
    CHECK_EQUAL(0, f.wnd.seq);
}
...
TEST(send_wnd, ack_first_seq_copies_cur_ack_vec)
{
    Fixture f;
    f.wnd.size = 1;
    arq__send_wnd_ack(&f.wnd, f.wnd.seq, 1u);
...
TEST(send_wnd, ack_the_entire_send_window)
{
    ...
    arq__send_wnd_ack(&f.wnd, 0, f.wnd.full_ack_vec);
    CHECK_EQUAL(1, f.wnd.size);
    CHECK_EQUAL(f.wnd.cap, f.wnd.seq);
}
